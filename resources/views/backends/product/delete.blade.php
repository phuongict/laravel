...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('backend.product.destroy', ['id' => request()->route()->id]) }}" method="post">
                @csrf
                <input type="hidden" name="id" value="{{ $objItem->id }}">
                <div class="row">
                    <div class="col-sm-12">
                        <div class="alert alert-danger">
                            <h6>{{ __('all.wh_delete', ['name' => 'product', 'del_name' => $objItem->name]) }}</h6>
                        </div>
                        <div class="form-group text-right">
                            <a id="btnCancel" href="{{ route('backend.product.index') }}" class="btn btn-default btn-sm"><i
