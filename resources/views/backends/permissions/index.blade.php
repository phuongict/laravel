        ...
        <div class="card-body">
            <form action="" method="get">
                <div class="row">
                    ...
                    <div class="col-md-3 col-sm-4 ">
                        <div class="form-group">
                            <input type="text" placeholder="slug" value="{{ old('slug') }}" name="slug" class="form-control form-control-sm">
                        </div>
                    </div>
                    <div class="col-md-3 col-sm-12 text-center">
                        <div class="form-group">
                            ...
                            <a class="btn btn-info btn-sm" href="{{  route('backend.permission.create') }}"> {{ __('all.add_new') }}</a>
                        </div>
                    </div>
                </div>
            </form>
                    ...
                    @if(isset($lists) && $lists->count() > 0)
                        @foreach($lists as $item)
                            <tr>
                                ...
                                <td>{{ $item->slug }}</td>
                                <td>
                                    {{ $item->parent }}. {{ $item->parent_name??'No parent' }}
                                </td>
                                <td>
                                    {{ date('d/m/Y H:i:s', strtotime($item->created_at)) }}
                                </td>
                                <td>
                                    <a href="{{ route('backend.permission.edit', ['id' => $item->id]) }}"><i class="fa fa-edit"></i></a>
                                    <a href="{{ route('backend.permission.delete', ['id' => $item->id]) }}"><i class="fa fa-trash" style="color: red;"></i></a>
                                </td>
                            </tr>
                        @endforeach
                    @endif
