                ...
                <div class="row">
                    <div class="col-sm-8">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="content" id="content" rows="5" placeholder="{{ __('post.content') }}"
                                      class="form-control textarea @error('content') is-invalid @enderror">{{ old('content') }}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="short_content">{{ __('post.short_content') }}</label>
                            <textarea name="short_content" id="short_content" placeholder="{{ __('post.short_content') }}"
                                      class="form-control @error('short_content') is-invalid @enderror">{{ old('short_content') }}</textarea>
                        </div>
                        <div class="form-group">
                    ...
                    <div class="col-sm-4">
                        ...
                        <div class="form-group">
                            ...
                            <select name="category_id" id="category_id" class="form-control select2" data-placeholder="{{ __('post.choose_category') }}">
                                <option value="">{{ __('post.choose_category') }}</option>
                                @foreach($listCategory as $category)
                                    <option value="{{ $category->id }}" @if(old('category_id') && $category->id == old('category_id')) selected @endif>{{ $category->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
