...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('backend.menu.update', ['id' => request()->route('id')]) }}" method="post">
                @csrf
                <div class="row">
                    <div class="col-sm-12">
                        ...
                        <div class="form-group">
                            @php
                                $type = old('type')??$objItem->type;
                            @endphp
                            @foreach(config('app.menu_type') as $key => $value)
                                <div class="form-check-inline">
                                    <div class="custom-control custom-radio">
                                        <input type="radio" class="custom-control-input" id="type_{{ $key }}" name="type" value="{{ $key }}" {{ $type==$key?'checked':'' }}>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="value" value="{{ old('value')??$objItem->value }}"
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="order" value="{{ old('order')??$objItem->order }}"
                                   class="form-control @error('order') is-invalid @enderror" id="order"
                                   placeholder="{{ __('menu.order') }}">
                        </div>
                        <div class="form-group">
                            <label for="icon">Icon (css class)</label>
                            <input type="text" name="icon" value="{{ old('icon')??$objItem->icon }}"
                                   class="form-control @error('icon') is-invalid @enderror" id="icon"
                                   placeholder="Icon">
                        </div>
                        <div class="form-group">
                            @php
                                $location = old('location')??$objItem->location;
                            @endphp
                            @foreach(config('app.menu_location') as $key => $value)
                                <div class="form-check-inline">
                                    <div class="custom-control custom-radio">
                                        <input type="radio" class="custom-control-input" id="location_{{ $key }}" name="location" value="{{ $key }}" {{ $location==$key?'checked':'' }}>
